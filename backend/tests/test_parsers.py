"""Tests for parsers.py against the real sample files."""
...
def test_every_sample_parses(path):
    """No real tabular file in the sample folder should raise on parse."""
    # Metadata sidecars are documentation/provenance files, not datasets.
    if path.name.endswith(".metadata.json"):
        pytest.skip("metadata sidecar is not a tabular sample")
